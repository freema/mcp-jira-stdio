...
export const OAUTH_CALLBACK_PORT = 7789;
const REDIRECT_URI = `http://localhost:${OAUTH_CALLBACK_PORT}/callback`;

const TOKEN_DIR = join(homedir(), '.mcp-jira-stdio');
...
let memCache: { token: string; cloudId: string; expiresAt: number } | null = null;
...
async function fetchCloudId(accessToken: string, siteUrl: string): Promise<string> {
...
async function authorize(
  ...
  const code = await new Promise<string>((resolve, reject) => {
    const server = createServer((req, res) => {
      ...
      if (error || returnedState !== state || !returnedCode) {
        ...
        reject(new Error(error ? `OAuth error: ${error}` : 'Invalid OAuth callback'));
        return;
      }
    ...
    server.listen(OAUTH_CALLBACK_PORT, () => {
      ...
      console.error('\n[Jira MCP] Authorization required.');
      console.error(
        `[Jira MCP] Opening browser... If it doesn't open, visit:\n\n  ${url}\n`,
      );
      const cmd =
  ...
  console.error('[Jira MCP] Authorization successful!\n');
...
export async function getOAuthToken(
  ...
  if (stored) {
    if (stored.expiresAt > Date.now() + BUFFER_MS) {
      memCache = { token: stored.accessToken, cloudId: stored.cloudId, expiresAt: stored.expiresAt };
    ...
    if (stored.refreshToken) {
      try {
        ...
        saveStore(store);
        memCache = { token: updated.accessToken, cloudId: updated.cloudId, expiresAt: updated.expiresAt };
  ...
  saveStore(store);
  memCache = { token: tokens.accessToken, cloudId: tokens.cloudId, expiresAt: tokens.expiresAt };
